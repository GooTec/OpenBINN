...
class InnvestigateModel(torch.nn.Module):
    ...
    def investigate_layer(self, in_tensor=None, rel_for_class=None):
        """
        Method for 'innvestigating' the model with the LRP rule chosen at
        the initialization of the InnvestigateModel.
        Args:
        ...
        Returns:
            Model output, relevances of nodes in the input layer, and relevances
        ...
        with torch.no_grad():
            # Check if investigation can be performed.
            if in_tensor is None and self.prediction is None:
                raise RuntimeError("Model needs to be evaluated at least "
                                "once before an investigation can be "
                                "performed. Please evaluate the model first "
                                "or call innvestigate with a new input to "
                                "evaluate.")
            ...
            # Determine the relevance target class.
            if rel_for_class is None:
                # Default: analyze for the class with the highest prediction.
                org_shape = self.prediction.size()
                self.prediction = self.prediction.view(org_shape[0], -1)
                max_v, _ = torch.max(self.prediction, dim=1, keepdim=True)
                only_max_score = torch.zeros_like(self.prediction).to(self.device)
            ...
            # Back-propagate relevance through the layers.
            rev_model = self.inverter.module_list[::-1]
            relevance = relevance_tensor.detach()
            del relevance_tensor

            # Dictionary to save relevance distributions per layer
            layer_relevances = {}
            relevance_per_layer = [relevance]
            for i, layer in enumerate(rev_model):
                relevance = self.inverter.compute_propagated_relevance(layer, relevance)
                relevance_per_layer.append(relevance.cpu())
                layer_relevances[f"Layer_{len(rev_model) - i}"] = relevance.cpu()

            self.r_values_per_layer = relevance_per_layer

            del relevance
            if self.device.type == "cuda":
                torch.cuda.empty_cache()

            return self.prediction, layer_relevances
