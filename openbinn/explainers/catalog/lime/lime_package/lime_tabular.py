...
from .discretize import VentileDiscretizer
...
class LimeTabularExplainer(object):
    ...
    def __init__(self,
                 ...
                 std=0.01):  # COMMENT FROM MP: added standard deviation for local samples
        """Init function.
    ...
    def explain_instance(self,
        ...
        if sp.sparse.issparse(data):
            ...
            # COMMENT FROM MP:
            # when we set "sample around instance = True", we do not want scale the perturbations using the mean and std
            # derived from the data as we wish to weight samples according to their local neighboorhood and not
            # according to data statistics
            # HENCE: added the if statement
            if self.sample_around_instance:
                scaled_data = data
                # print(np.var(scaled_data, axis=1))
            else:
    ...
    def __data_inverse(self,
        ...
        if self.discretizer is None:
            ...
            if self.sample_around_instance:
                ...
                # MP: changed to do truly local sampling using fixed std
                
                data = data * self.std + instance_sample
