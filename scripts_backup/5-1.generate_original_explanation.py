...
of the original (no-bootstrap, no-permutation) PNet model on the test split.

원본 코드를 그대로 살리고, 반복(loop)만 제거해 한 번만 실행하도록 간략화했습니다.
"""
...
def explain_one_variant(seed_dir: Path, method: str):
    ...
    model_name = "PNet"
    data_name  = seed_dir.name        # 원본 모델 폴더명
    split_name = "test"
...
def main(args):
    # bootstrap/gene-perm 반복 제거: 단일 폴더만 처리
    scenario = Path(args.scenario)
    explain_one_variant(scenario, method=args.method)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--scenario", required=True,
                        help="Original model folder containing splits/ and results/")
