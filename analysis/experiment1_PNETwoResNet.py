...
from openbinn.binn import PNetNoResidual
...
def train_dataset(
    ...
    torch.save(state, results_dir / "optimal" / PNET_NORES_MODEL_FILENAME)
...
def explain_dataset(data_dir: Path, results_dir: Path, reactome, maps, args=None):
    ...
    state = torch.load(results_dir / 'optimal' / PNET_NORES_MODEL_FILENAME, map_location='cpu')
    if isinstance(state, dict) and 'state_dict' in state:
        state = state['state_dict']
    ...
    explain_root = results_dir / 'explanations' / 'PNETNoRes'
    ...
    for method in methods:
        ...
        for idx, (lname, arrs) in enumerate(expl_acc.items()):
            ...
            out_fp = explain_root / f"PNetNoRes_{data_label}_{method}_L{idx+1}_layer{idx}_test.csv"
...
if __name__ == "__main__":
    ...
    if args.lr_list or args.bs_list:
        ...
        maps = best_maps
        (results_dir / "optimal").mkdir(parents=True, exist_ok=True)
        torch.save(best_state, results_dir / "optimal" / PNET_NORES_MODEL_FILENAME)
        with open(results_dir / "optimal" / PNET_NORES_CONFIG_FILENAME, "w") as f:
